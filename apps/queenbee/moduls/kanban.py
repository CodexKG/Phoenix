...
#         if card_form.is_valid() and attachment_form.is_valid():
#             # Создаем карточку
...
#             # Получаем все файлы из поля формы и сохраняем каждый файл как отдельное вложение
#             files = request.FILES.getlist('file')
#             for file in files:
#                 models.Attachment.objects.create(
#                     card=card, user=request.user, file=file)

#             return JsonResponse({'success': True, 'card_title': card.title, 'card_description': card.description, 'card_id': card.id})
#         else:
#             if not card_form.is_valid():
#                 print("Ошибки формы карточки:", card_form.errors.as_json())

#             if not attachment_form.is_valid():
#                 print("Ошибки формы вложений:",
#                       attachment_form.errors.as_json())
#             return JsonResponse({'success': False, 'errors': card_form.errors})
...
#             return JsonResponse({'success': True, 'card_title': card.title, 'card_description': card.description, 'card_id': card.id})
#         else:
#             # Возврат ошибок обеих форм, если они есть
#             return JsonResponse({'success': False, 'errors': {
#                 'card_form': card_form.errors,
...
#                 card.list = list_obj
#                 card.position = list_obj.card_lists.count() + 1
#                 card.save()  # Попытка сохранить новую карточку
#                 card_form.save_m2m()
